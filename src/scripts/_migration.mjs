...
Hooks.once("ready", async () => {
  if (
    ...
    ui.notifications.info(
      `<div class="${MODULE.ID}-migration-tooltip-203"><strong>${MODULE.TITLE}:</strong> ${MODULE.localize("migration.v204.starting")}</div>`,
      { permanent: true },
    );
    ...
    for (const key of Object.keys(MODULE.setting("lockedSettings"))) {
      ...
      if (
        (settingDetails?.namespace ?? false) &&
        (settingDetails?.namespace ?? false) &&
        (game.settings.get(settingDetails.namespace, settingDetails.key) ??
          false)
      ) {
        newLockedSettings[`${settingDetails.namespace}.${settingDetails.key}`] =
          game.settings.get(settingDetails.namespace, settingDetails.key);
      }
    }
  ...
  if (
    ...
    Dialog.confirm({
      ...
      render: (elem) =>
        elem[0]
          .closest(".app.dialog")
          .querySelector("a.header-button.close")
          .remove(),
      yes: () => true,
      no: () => false,
    }).then(async (response) => {
  ...
  if (
    ...
    ui.notifications.info(
      `<strong>${MODULE.TITLE}:</strong> ${MODULE.localize("migration.v214.smartLabels")}`,
    );
    await MODULE.setting(
      "smartLabels",
      MODULE.setting("autoPrefixModules") ?? true,
    );

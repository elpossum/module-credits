...
export class PreviewDialog extends FormApplication {
  ...
  activateListeners(html) {
    ...
    const navElements = html[0].querySelectorAll(
      "aside.sidebar nav ol.directory-list li.directory-item",
    );
    navElements.forEach((navElem) => {
      navElem.querySelectorAll("span").forEach((spanElem) => {
        spanElem.addEventListener("click", (event) => {
          ...
          navElements.forEach((activeElem) =>
          ...
          game.modules
            ...
            .API.getContent(
              this.IsSystem
                ? game.system
                : game.modules.get(elem.dataset.moduleId),
              elem.dataset.fileType,
            ...
            .then((response) => {
              ...
              html[0].querySelector(
                "section.journal-entry-content .journal-entry-page",
              ).innerHTML = content;

              // Update Module as Seen
              elem
                .querySelector(".page-ownership i.fa-regular")
              ...
              if (
                game.user.isGM &&
                (MODULE.setting("trackedChangelogs")[elem.dataset.moduleId] ??
                  false)
              ) {
                MODULE.setting(
                  "trackedChangelogs",
          ...
          // Disable Buttons
          html[0]
            .querySelectorAll("aside.sidebar .action-buttons button")
            .forEach((btn) => (btn.disabled = false));
          if (navElements[0].classList.contains("active"))
            html[0].querySelector(
              'aside.sidebar .action-buttons button[data-action="previous"]',
            ).disabled = true;
          if (navElements[navElements.length - 1].classList.contains("active"))
            html[0].querySelector(
              'aside.sidebar .action-buttons button[data-action="next"]',
            ).disabled = true;
        });
      });
    });
    ...
    collapseElem.addEventListener("click", (event) => {
      ...
      button.dataset.tooltip = sidebarCollapsed
        ? "JOURNAL.ViewExpand"
        : "JOURNAL.ViewCollapse";
      const i = button.children[0];
      i.setAttribute(
        "class",
        `fa-solid ${sidebarCollapsed ? "fa-caret-left" : "fa-caret-right"}`,
      );
      game.tooltip.deactivate();
    });

    // Bind Search Input
    const searchInput = html[0].querySelector(
      'aside.sidebar .directory-header input[type="text"]',
    );
      ...
      navElements.forEach((navElem) => {
        ...
        if (searchFor == "") {
          ...
          navElem
            ...
            .indexOf(searchFor) == -1 &&
          !navElem.classList.contains("active")
        ) {
    ...
    const nextButton = html[0].querySelector(
      'aside.sidebar .action-buttons button[data-action="next"]',
    );
    const previousButton = html[0].querySelector(
      'aside.sidebar .action-buttons button[data-action="previous"]',
    );
    ...
    previousButton.addEventListener("click", (event) => {
      html[0]
        ...
        .previousElementSibling.querySelector(".page-title")
        .click();
    });

    // Mark as Seen
    const markAsSeen = html[0].querySelector(

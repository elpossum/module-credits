  ...
  async function expandedModuleDependencies(event) {
    ...
    const getValidRelationship = (relationship, isOptional) =>
      relationship.reduce((filtered, requirement) => {
        ...
        // If Enabling
        if (input.checked && isEnabled(requirement.id))
          return filtered.concat([]);
        ...
        if (!input.checked) {
          let requirements = allPackages.reduce(
            (relationships, aRequirement) => {
              ...
              if (
                (aRequirement?.type ?? "").toLowerCase() == "module" &&
                !isEnabled(aRequirement?.id)
              )
                return relationships.concat([]);

              return relationships.concat(
                Array.from(aRequirement?.relationships?.requires ?? [])
                  .concat(
                    Array.from(
                      aRequirement?.relationships?.optional ??
                        Array.from(
                          aRequirement?.relationships?.flags?.optional ?? [],
                        ),
                    ),
                  )
                  .filter((relationship) => {
                    return (
                      requirement.id == relationship.id &&
                      isEnabled(relationship.id)
                    );
                  }) ?? [],
              );
            },
        ...
        if (isEnabled(requirement.id) == input.checked)
          return filtered.concat([]);
    ...
    const recommends = getValidRelationship(
      Array.from(
        module?.relationships?.recommends ??
          Array.from(module?.relationships?.flags?.recommends ?? []),
      ),
      true,
    );
    const optionals = getValidRelationship(
      Array.from(
        module?.relationships?.optional ??
          Array.from(module?.relationships?.flags?.optional ?? []),
      ),
      true,
    );
    ...
    return Dialog[MODULE.setting("dependencyDialogType")]({
      ...
      [`${MODULE.setting("dependencyDialogType") == "prompt" ? "callback" : "yes"}`]:
        (elem) => {
          MODULE.log("callback", elem);
          elem[0]
            .querySelectorAll('input[type="checkbox"]')
            .forEach((requirementInput) => {
              if (requirementInput.checked) {
                input.form.querySelector(
                  `#module-list .package[data-module-id="${requirementInput?.name}"] input[type="checkbox"]`,
                ).checked = input.checked;

                // HANDLE FOR MM+ SPECIFIC FUNCTIONALITY
                input.form
                  .querySelector(
                    `#module-list .package[data-module-id="${requirementInput?.name}"]`,
                  )

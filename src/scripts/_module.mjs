export class MODULE {
  ...
  static CONSOLE(LOG_LEVEL, ...args) {
    try {
      if (
        game.modules
          .get("_dev-mode")
          ?.api?.getPackageDebugValue(this.ID, "level") >= LOG_LEVEL
      ) {
        console.log(
  ...
  static setting(...args) {
    ...
    if (args[0].toLowerCase() == "register") {
      ...
      const setting = args[1]; // This is the name of the setting
      const value = args[2]; // This is the settings of the setting
